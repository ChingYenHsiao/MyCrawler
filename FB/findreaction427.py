...
def main():
    ...
    #FanpageList = open("./fanpagelist",'r')
    #FanpageList = open("./fanpagelist2",'r')
    FanpageList = open("./traverse",'r')
    #mylist = FanpageList.readlines()
    mylist = [ "./data/20161228/10155824496656065"]
    for page in mylist:
        ...
        dirName = p[2]
        Page_dirName = p[3]
        if(Page_dirName +"_reaction.csv" in finished_list):
            continue
        if(os.path.exists(Path+dirName+"/"+Page_dirName+"/after20160226/auto_post_list")):
            with open(Path+dirName+"/"+Page_dirName+"/after20160226/auto_post_list") as post_list_file:
                post_list = post_list_file.readlines()

                #check finished pages
