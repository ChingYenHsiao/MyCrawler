...
def main():
    ...
    until = 'until=2017-05-24'
    since = 'since=2017-04-07'

    Path = "../../data/"
    dir_layer1 = ""
    dir_layer2 = ""
    graph = facebook.GraphAPI(access_token = 'EAAXfzQZBz8EMBAEZCdBTSpMciqzw7xDAP5Ugkqvy85XvCaY62QOfU6vMR11ERlWv8zS9Utvd4cfqX7ypmHDD5ZCYp3YQaZAYqYhRUrxO8A5rJF0HMae3PObZB0LhQ26Nudz2EhNLgzpVXgVoSj5UWgSEBGnrJgAAZD', version = '2.7')
    graph2 = facebook.GraphAPI(access_token = 'EAAXfzQZBz8EMBAEZCdBTSpMciqzw7xDAP5Ugkqvy85XvCaY62QOfU6vMR11ERlWv8zS9Utvd4cfqX7ypmHDD5ZCYp3YQaZAYqYhRUrxO8A5rJF0HMae3PObZB0LhQ26Nudz2EhNLgzpVXgVoSj5UWgSEBGnrJgAAZD', version = '2.8')
    start = False
    total_page_count = 0
    ...
    mylist = FanpageList.readlines()
    for fileName in mylist:
        fileName = fileName.strip()
# # for  root, dirs, fileNames in os.walk(Path+"Post"):
# #     for fileName in fileNames:
#         #貴婦奈奈_111249032235947.csv
#         if (fileName == "貴婦奈奈_111249032235947.csv"):
#             start =True
#         if start!=True:
#             continue
        ...
        if(len(tmp[0])!=0):
            ...
            page_id = tmp[1].split(".")[0]
            with open(Path+"Post_reaction522"+"/"+fileName,'a') as Post_csv:
                        ...
                        if(resp!='null'  and len(resp)>1):
                            ...
                            if((len(resp) > 1)):
                                if(len(resp['paging']) > 1):
                                    ...
                                    print (next_page)
                                    get_next_post(next_page,Post_dict) #找下一30筆post
                            get_post_comment(Path+"comment_reaction522/",Post_dict,graph)     #找 Post 的 comments
                            

                            reaction_list = ["LIKE","LOVE","WOW","HAHA","SAD","ANGRY","THANKFUL"]    #找每個reaction有多少人按
                                ...
                                if (error_flag_1) == False:
                                    ...
                                    if len(resp2) > 1:
                                        if(len(resp2['paging']) > 1):
                                            ...
                                            get_next_reaction(next_page_reaction,reaction_list[l],Post_dict)   #繼續找reaction
...
def get_resp(path,graph,limit_number):
    resp = 'null'
    try:
        resp = graph.get_object(path,limit=limit_number)   #每次300個 可能會壞掉
        return resp
    except  facebook.GraphAPIError as g:
        gstr = "Object with ID" 
        ...
        if (gstr in g.message) & (gstr2 in g.message):
            ...
            while k == 0:
                time.sleep(5)
                try:
                    resp_2 = graph.get_object(path,limit=100)    #自動減量 可自訂
                    error_flag3 = 0
                    k = k + 1
                    return resp
                except facebook.GraphAPIError as g2:
...
def get_post_comment(result_path,Post_dict,graph):
    ...
    with open(result_comment, 'a') as outfile:
        ...
        for post_id in Post_dict:
            ...
            resp_2 = get_resp(comment_path,graph,300)  #每次300個 可能會壞掉
            ...
            if int(error_flag3) == 0:
                ...
                for comment in ob:
                    ...
                    if int(resu_comcom) > 50:
                        ...
                        if len(next_ob_page['paging']) > 1:
                            ...
                            get_com_com(next_comcom_page,outfile,post_id)    #找subcomment
                ...
                if(len(resp_2) > 1):
                    if(len(resp_2['paging']) > 1):
                        ...
                        ob_len+= get_next_comment(next_page,post_id,n,graph,outfile)  #找下N筆comment
    ...
    if int(error_flag5) == 0:
        ...
        if(len(next_comcom) > 1):
            if(len(next_comcom['paging']) > 2):
                ...
                get_next_comcom(next_page,post_id,n,outfile)  #找下N筆subcomment
